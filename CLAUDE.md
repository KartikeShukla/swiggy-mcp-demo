...
- Retry policy: max 2 retries with exponential backoff (500ms base, 2x, 20% jitter, 5s cap) for statuses 429/500/502/503/504/529
...
- Context management: auto-cleans old tool results when input > 25k tokens, keeps last 5
...
src/integrations/anthropic/
  ...
  retry-policy.ts       — Retryable error detection (429/5xx/529) and exponential backoff (500ms base, 2x, 20% jitter, 5s cap)
```
...
4. `buildMessageStreamParams()` sanitizes messages (capped at `MAX_CONTEXT_MESSAGES=24`), builds system blocks with `cache_control`, configures MCP servers
...
- **Context management**: `clear_tool_uses_20250919` with trigger at 25k tokens, keeps last 5 tool uses, `clear_at_least: 2000` tokens to avoid cache invalidation for small clearings
...
- **Retry policy**: exponential backoff (500ms base, 2x multiplier, 20% jitter, 5s cap), max 2 retries for statuses 429, 500, 502, 503, 504, 529
