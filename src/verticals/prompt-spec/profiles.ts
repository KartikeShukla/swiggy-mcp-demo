...
export const foodPromptProfile: PromptProfile = {
  ...
  phaseFlow: [
    ...
    "Direct mode: search named products immediately with the most specific query (brand/size/variant).",
    ...
    "Only after explicit sourcing/shopping ask: search one ingredient per turn and confirm before the next.",
  ...
  toolPolicies: [
    "Search with specific name + quantity (e.g. 'paneer 500g'). Prefer brand/size/variant specificity.",
    "In advisory mode, do not call tools unless user explicitly asks to source/shop.",
    ...
    "Add-to-cart is incremental; never clear cart.",
...
export const stylePromptProfile: PromptProfile = {
  ...
  phaseFlow: [
    ...
    "Only after explicit shopping ask: search one product per turn and confirm before the next.",
  ...
  toolPolicies: [
    "After explicit shopping ask, search one product per turn. Always qualify the product type — e.g. 'face wash for oily skin', 'shampoo for dry hair', not just 'face wash' or 'shampoo'.",
...
export const diningPromptProfile: PromptProfile = {
  ...
  preToolRequirement:
    "Before search, require cuisine_or_vibe + party_size. If active address exists in system context, use it as location unless user asks otherwise. Use datetime block for relative time references.",
  phaseFlow: [
    "Collect dining constraints; use active address as location if available.",
    "Run one focused restaurant discovery call with cuisine, area, and budget when specified.",
    "Dish-only query: map dish to cuisine (dosa -> South Indian) and mention proxy.",
    "Use strict-first cuisine/vibe/location matching. If no exact combined match exists, show closest options and ask to relax one filter.",
    "Present cuisine/area cards and wait for user selection.",
    "After selection, ALWAYS check availability for requested date/time and party size before attempting to book.",
    "Let user choose from returned slots; never assume a requested slot exists. Prioritize near-time slots first.",
    "When restaurant/slot metadata is provided, use it directly. Ask one final explicit yes/no before booking tool call.",
  ...
  toolPolicies: [
    ...
    "Do not call availability tools for a restaurant unless the user explicitly selected that restaurant.",
    "Do not call booking tools until user selects a specific slot and gives final explicit yes/no.",
    "One search per step; show results and wait for user input.",
    "If strict matches fail, ask to relax one filter and still show closest options.",
    "Keep claims card-grounded: do not claim an exact match or exact slot success unless that result is confirmed by current tool output.",
  ],
  responseStyle: [
    "Keep suggestions practical and local-aware.",
    "Keep booking copy short.",
  ],
  confirmationRules: [
  ...
  fallbackRules: [
    ...
    "When strict matches fail, show closest options and ask which filter to relax next.",
